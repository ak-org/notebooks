...
text2Corpus = function() {
  ...
  write.table(freq[ord],"word_freq.csv", col.names = FALSE)
...
runlda = function(datamatrix, topics, tc) {
  ...
  controllist5 = list(alpha = 50/k, estimate.beta = TRUE,
                     ...
                     best = TRUE, burnin = 100, iter = 1000, thin = 50)
  controllist1 = list(alpha = 50/k, estimate.beta = TRUE,
                      nstart = 1, seed = list(44773),
                      best = TRUE, burnin = 100, iter = 1000, thin = 50) 
  
  print("Start LDA")
  ldaOut = LDA (datamatrix, k, method = "Gibbs", control = controllist5)
  print("write topics")
  ldaOut.topics = as.matrix(topics(ldaOut))
  write.csv(ldaOut.topics, file=paste("LDAGibbs", k, "DocsToTopics.csv",sep=""))
  ldaOut.terms <- as.matrix(terms(ldaOut,termcount))
  write.csv(ldaOut.terms,file=paste("LDAGibbs",k,"TopicsToTerms.csv",sep=""))
  topicProbabilities <- as.data.frame(ldaOut@gamma)
  write.csv(topicProbabilities,file=paste("LDAGibbs",k,"TopicProbabilities.csv",sep=""))
  ...
  write.csv(topic1ToTopic2,file=paste("LDAGibbs",k,"Topic1ToTopic2.csv",sep=""))
  write.csv(topic2ToTopic3,file=paste("LDAGibbs",k,"Topic2ToTopic3.csv",sep=""))
